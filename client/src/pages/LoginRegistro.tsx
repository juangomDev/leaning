...
export const LoginRegistro: React.FC = () => {
  ...
  const [panel, setPanel] = useState<string>(searchParams.get('mode') === 'register' ? 'register' : 'login');
  const [role, setRole] = useState<string>(searchParams.get('role') === 'tutor' ? 'tutor' : 'student');
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        setErrorMsg(res.error.message || 'Credenciales inválidas');
      } else {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        setErrorMsg(res.error.message || 'No se pudo crear la cuenta');
      } else {
