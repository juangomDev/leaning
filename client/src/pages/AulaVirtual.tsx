...
export const AulaVirtual: React.FC = () => {
  ...
  const [whiteboardMode, setWhiteboardMode] = useState<boolean>(false);
  const [handRaised, setHandRaised] = useState<boolean>(false);
  const [sidebarOpen, setSidebarOpen] = useState<boolean>(true);
  const [sidebarTab, setSidebarTab] = useState<'chat' | 'participantes'>('chat');
  ...
  // Live Timer
  const [seconds, setSeconds] = useState<number>(1122); // 18m 42s
  useEffect(() => {
